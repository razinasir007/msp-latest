...
export const EditedImages = React.forwardRef(
  (
    props: {
      OrderLoading;
      presentationModeEdited?;
      viewImageEdited?;
      setViewImageEdited;
      ...
      fullscreen;
      editedPhotos;
      slideShowIndexEditedImages;
      serachItem;
      handleDoubleClickEdited;
    },
    ref: React.Ref<any>
  ) => {
    return (
      <Box mt='20px'>
        ...
        ) : props.presentationModeEdited || props.viewImageEdited ? (
          <>
            <Flex justifyContent='space-between' mb='20px' mt='20px'>
              <Text fontSize='h5' fontWeight='400'>
                Presentation View For Edited Images
              </Text>
              <MdClose
                size={30}
                style={{ cursor: "pointer" }}
                onClick={() =>
                  props.viewImageEdited
                    ? props.setViewImageEdited(false)
                    : props.setLaunchPresentationModeEdited(false)
                }
              />
            </Flex>
            <Box>
              <FullScreen handle={props.handle} onChange={props.handleChange}>
                <Splide
                  ...
                  options={{
                    type: "loop",
                    start: props.viewImageEdited
                      ? props.slideShowIndexEditedImages
                      : 0,
                  }}
                  aria-label='React Splide Example'
                >
                  {props.editedPhotos.map((image) => (
                  ...
                  <Box key={ele.id}>
                    <VStack>
                      <img
                        ...
                        style={{ cursor: "pointer" }}
                        onDoubleClick={() =>
                          props.handleDoubleClickEdited(index)
                        }
                      />
