...
import {
  CreateCheckoutForm,
  GetCheckoutFormByLocation,
} from "../../../apollo/formsQueries";
import { FormPdf } from "../shared/formPdf/formPdf";
...
export const OrderCheckoutForm = (props) => {
  const navigate = useNavigate();
  const filledFormId = uuidv4();
  const user = useHookstate(globalState.user);
  const orgLogo = user!.value!.organization!.logo;
  const orgName = user!.value!.organization!.name;
  ...
  const {
    ...
    error: fieldsError,
    data: fieldsData,
  } = useQuery(GetCheckoutFormByLocation, {
    variables: { name: "CHECKOUT_FORM", locId: user!.value!.storeLocId },
  });
  ...
  //send filled out form to backend
  const [
    sendCheckoutForm,
    { loading: sendCheckoutFormLoading, data: sendCheckoutFormData },
  ] = useMutation(CreateCheckoutForm);
    ...
    if (formSubmitted) {
      ...
      (async function () {
        try {
          ...
          const pdfFile = new File([pdfBlob], "Checkout Form.pdf", {
          ...
          sendCheckoutForm({
            ...
            onCompleted: (resp) => {
              ...
              props.setCheckoutFormSubmitted(true);
              Swal.fire({
                icon: "success",
  ...
  useEffect(() => {
    if (fieldsData?.organizationForms?.getItemsByLocationAndName?.data) {
      const filteredFields =
        fieldsData?.organizationForms?.getItemsByLocationAndName?.data.map(
          (field) => {
            if (field.type === "client") {
  ...
  useEffect(() => {
    if (ClientDetails?.clients?.lookupClient) {
      ...
      updatedFieldValues[0].value =
        ClientDetails?.clients?.lookupClient.firstname;
      updatedFieldValues[1].value =
        ClientDetails?.clients?.lookupClient.lastname;
      updatedFieldValues[2].value = ClientDetails?.clients?.lookupClient.email;
      updatedFieldValues[3].value =
        ClientDetails?.clients?.lookupClient.phoneNumber;
      updatedFieldValues[4].value =
        ...
        <Flex width='100%' alignItems='flex-start' gap='8px' key={index}>
          <VStack gap='2px' w='97%' alignItems='left'>
            <Link
              fontWeight='semibold'
              onClick={() => {
                console.log("clicked", field.data);
          ...
          <form onSubmit={handleSubmit}>
            ...
            <CardBody padding='16px'>
              <VStack width='100%' spacing='15px'>
                {fieldsLoading ? (
                ...
                ) : fieldsData?.organizationForms?.getItemsByLocationAndName
                    ?.data.length ? (
                  ...
                  <VStack spacing='16px' alignItems={"center"}>
                    <Text fontSize='p4' fontWeight='medium' textAlign='center'>
                      Please create a checkout form in Customer Management.
                      Checkout Form is a crucial part of our Checkout process.
                    </Text>
                    <Button
                      onClick={() => {
                        navigate(
                          `${ROUTE_PATHS.LAYOUT}/${ROUTE_PATHS.SETTINGS}/${ROUTE_PATHS.CUSTOMER}`
                        );
                      }}
                    >
            ...
            <CardFooter>
              <Flex w='100%' justifyContent='right'>
                {fieldsData?.organizationForms?.getItemsByLocationAndName?.data
                  .length && (
                  <Button
                    isDisabled={props.checkoutFormSubmitted}
                    type='submit'
                    isLoading={formSubmissionLoader}
                  >
                    {props.checkoutFormSubmitted ? "Submitted" : "Submit"}
                  </Button>
                )}
              </Flex>
            </CardFooter>
